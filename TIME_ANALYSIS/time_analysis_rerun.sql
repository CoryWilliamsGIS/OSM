...
nairobi_
kenya_
kenya_nairobi (prior to running st_contains to only include Nairobi edits)... new files = nairobi2018_
*/

--TOTAL TABLE COUNT 
select count(*) from public.kenya2018_point;

--TOTAL TABLE COUNT WITHIN NAIROBI (SANITY CHECK)
select count(*) from public.kenya2018_point, public.nairobi_consts
where st_contains(public.nairobi_consts.geom, public.kenya2018_point.way);

--CREATE A NEW TABLE ONLY INCLUDING NAIROBI EDITS 
create table nairobi2018_point as (
select * from public.kenya2018_point, public.nairobi_consts
where st_contains(public.nairobi_consts.geom, public.kenya2018_point.way)
);

create table nairobi2018_line as (
select * from public.kenya2018_line, public.nairobi_consts
where st_contains(public.nairobi_consts.geom, public.kenya2018_line.way)
);

create table nairobi2018_polygon as (
select * from public.kenya2018_polygon, public.nairobi_consts
where st_contains(public.nairobi_consts.geom, public.kenya2018_polygon.way)
);

--delete zeros whilst making a note of the number of records deleted 
delete from nairobi2018_point where osm_uid = '0'; --0
delete from nairobi2018_line where osm_uid = '0'; --0
delete from nairobi2018_polygon where osm_uid = '0'; --0
...
from public.nairobi2018_point
group by osm_timestamp::date, osm_uid
order by count(*) desc;

select osm_timestamp::date, osm_uid, count(*)
from public.nairobi2018_line
group by osm_timestamp::date, osm_uid
order by count(*) desc;

select osm_timestamp::date, osm_uid, count(*)
from public.nairobi2018_polygon
group by osm_timestamp::date, osm_uid
order by count(*) desc;

--nairobi2018_point = 7523 bulk edits to be removed
--nairobi2018_line = 0 bulk edits to be removed
--nairobi2018_polygon = 0 bulk edits to be removed


--INFERRING LOCALS
create table n_2018_local as (
select osm_uid, 
...
((select sum(coalesce(points_total, 0.0000000000000000000000001)) + sum(coalesce(line_total, 0.0000000000000000000001)) + sum(coalesce(polygon_total, 0.000000001)))) *100),2) as percent
	from (
	select * from (
select osm_uid from public.kenya2018_point
    union 
	select osm_uid from public.kenya2018_line
	union
	select osm_uid from public.kenya2018_polygon)
	as kosm_uid
) tbl0
inner join
(
	SELECT osm_uid as point_osm_uid, count(*) as points_within
FROM kenya2018_point, nairobi_consts
WHERE ST_within(kenya2018_point.way, nairobi_consts.geom)
	group by osm_uid
) tbl1
on tbl0.osm_uid = tbl1.point_osm_uid
inner join
(	SELECT osm_uid as osm_uid2, count(*) as points_total
FROM kenya2018_point
 group by osm_uid
) tbl2
...
FROM kenya2018_line, nairobi_consts
WHERE ST_within(kenya2018_line.way, nairobi_consts.geom)
	group by osm_uid
) tbl3
...
FROM kenya2018_polygon, nairobi_consts
WHERE ST_within(kenya2018_polygon.way, nairobi_consts.geom)
	group by osm_uid
) tbl5
on tbl0.osm_uid = tbl5.polygon_osm_uid
full outer join 
(	SELECT osm_uid as osm_uid2, count(*) as polygon_total
FROM kenya2018_polygon
 group by osm_uid
) tbl6
...
select osm_uid, percent from n_2018_local
...
--create POI table 
create table nairobi2018_poi as (
select * from nairobi2018_point where
...
/*
----848 users (total, not just within)
--850 within 
--CLOSE ENOUGH
SELECT * from (
(SELECT distinct osm_uid FROM public.nairobi2018_point, nairobi_consts WHERE ST_within(nairobi2018_point.way, nairobi_consts.geom))
union
(SELECT distinct osm_uid FROM public.nairobi2018_line, nairobi_consts WHERE ST_within(nairobi2018_line.way, nairobi_consts.geom)) 
union
(SELECT distinct osm_uid FROM public.nairobi2018_polygon, nairobi_consts WHERE ST_within(nairobi2018_polygon.way, nairobi_consts.geom)))
as x
*/