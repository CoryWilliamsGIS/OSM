...
--Number of nodes
SELECT COUNT(*) FROM public.manchester_final_point;
...
from (
    select osm_uid from public.manchester_finalfilt_point
    union 
	select osm_uid from public.manchester_finalfilt_line
	union
	select osm_uid from public.manchester_finalfilt_polygon
) as userid

--top 10 unique users
select a.osm_user, a.osm_uid, count(*) as num
FROM (SELECT osm_user, osm_uid FROM public.manchester_point UNION SELECT osm_user FROM public.manchester_line) AS a
...
SELECT
(SELECT COUNT(*) FROM PUBLIC.manchester_final_point where osm_version = '1')+
(SELECT COUNT(*) FROM PUBLIC.manchester_final_line where osm_version = '1')+
(SELECT COUNT(*) FROM PUBLIC.manchester_final_polygon where osm_version = '1')
as new_edits

SELECT(
(SELECT
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_point WHERE osm_version = '1')+
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_line WHERE osm_version = '1')+
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_polygon WHERE osm_version = '1')
as new_total) /
(SELECT		
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_point)+
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_line)+
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_polygon)) 
*100)
as percent_new

--Count of buildings  
select * from nairobi_polygon_final where building is not null and building != 'no';
SELECT COUNT (*) FROM nairobi_polygon_final where building is not null and building != 'no';
...
SELECT COUNT (*) FROM nairobi_polygon_final where building is not null and building != 'no' and osm_id > 0 and ("addr:housenumber" is not null or "addr:housename" is not null or "addr:street" is not null);
...
--Building Area
SELECT sum(ST_Area(way)/1000) as buildingAreaSqKm from nairobi_polygon_final where building is not null and building != 'no';
...
select osm_uid, count(osm_uid) as aerialwayEdits from nairobi_point_final where aerialway is not null group by osm_uid order by aerialwayEdits desc;
select osm_uid, count(osm_uid) as aerowayEdits from nairobi_point_final where aeroway is not null group by osm_uid order by aerowayEdits desc;
select osm_uid, count(osm_uid) as amenityEdits from nairobi_point_final where amenity is not null group by osm_uid order by amenityEdits desc;
select osm_uid, count(osm_uid) as barrierEdits from nairobi_point_final where barrier is not null group by osm_uid order by barrierEdits desc;
select osm_uid, count(osm_uid) as highwayEdits from nairobi_point_final where highway is not null group by osm_uid order by highwayEdits desc;
select osm_uid, count(osm_uid) as historicEdits from nairobi_point_final where historic is not null group by osm_uid order by historicEdits desc;
select osm_uid, count(osm_uid) as leisureEdits from nairobi_point_final where leisure is not null group by osm_uid order by leisureEdits desc;
select osm_uid, count(osm_uid) as lockEdits from nairobi_point_final where "lock" is not null group by osm_uid order by lockEdits desc;
select osm_uid, count(osm_uid) as man_madeEdits from nairobi_point_final where man_made is not null group by osm_uid order by man_madeEdits desc;
select osm_uid, count(osm_uid) as naturalEdits from nairobi_point_final where "natural" is not null group by osm_uid order by naturalEdits desc;
select osm_uid, count(osm_uid) as powerEdits from nairobi_point_final where power is not null group by osm_uid order by powerEdits desc;
select osm_uid, count(osm_uid) as railwayEdits from nairobi_point_final where railway is not null group by osm_uid order by railwayEdits desc;
select osm_uid, count(osm_uid) as shopEdits from nairobi_point_final where shop is not null group by osm_uid order by shopEdits desc;
select osm_uid, count(osm_uid) as tourismEdits from nairobi_point_final where tourism is not null group by osm_uid order by tourismEdits desc;
select osm_uid, count(osm_uid) as waterwayEdits from nairobi_point_final where waterway is not null group by osm_uid order by waterwayEdits desc;
...
--asuming manchester would be more detailed than nairobi_line_final
...
SELECT COUNT (*) FROM manchester_final_polygon where building is not null and building != 'no' and ("addr:housenumber" is not null or "addr:housename" is not null or "addr:place" is not null);

--number of religion tags
SELECT count(*) FROM manchester_final_polygon where religion is not null 
SELECT count(*) FROM manchester_final_point where religion is not null
...
select count(*) from manchester_finalfilt_point where amenity is null --737104
select count(*) from manchester_finalfilt_line where amenity is null --"38897"
select count(*) from manchester_finalfilt_polygon where amenity is null --"70300"

select count(*) from manchester_finalfilt_point --740681
select count(*) from manchester_finalfilt_line  --39135
select count(*) from manchester_finalfilt_polygon  --73503


SELECT(
(SELECT
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_point WHERE amenity is null)+
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_line WHERE amenity is null)+
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_polygon WHERE amenity is null)
as null_total) /
(SELECT		
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_point)+
(SELECT COUNT(*)::FLOAT FROM public.manchester_final_line)+
